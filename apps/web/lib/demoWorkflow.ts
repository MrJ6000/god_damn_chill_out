...
export async function executeApproved(
  ...
  const approved = plan.intents.flatMap((intent) => {
    const decision = decisionsByIntent.get(intent.intent_id);
    return decision?.verdict === "ALLOW" ? [{ decision, intent }] : [];
  });

  if (approved.length === 0) {
    throw new ApiClientError("NO_APPROVED_PAYMENTS", "No ALLOW payment is available to execute.");
  }

  if (plan.source === "mock") {
    const records = approved.map(({ decision, intent }) => ({
  ...
  for (const { decision, intent } of approved) {
    try {
      records.push({ ...await client.executePayment(intent, decision), source: "api" });
    } catch (error) {
      const notice = toDemoNotice(`執行 ${intent.invoice_id}`, error);
      const cachedOutcome = createMockPaymentOutcome(intent, decision);
      notices.push(notice);
      cachedDemoRecords.push({
        ...
        receipt: {
          ...
          payment_id: `CACHED-${intent.intent_id}`,
        },
        source: "mock",
      });
      notices.push({
        code: "EXECUTION_HALTED",
        message: "前一筆付款結果不明，為避免重複或超出授權，後續付款未再送出。",
        operation: "執行已核准付款",
      });
      break;
    }
